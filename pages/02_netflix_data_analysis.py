...
fill_nan = movies_df["country"].fillna("Unknown")
all_countries_string = ", ".join(fill_nan) 
all_countries_list = all_countries_string.split(", ")
unique_contries = set(all_countries_list)
...
top_10_countries = (
    movies_df
    .loc[movies_df["release_year"] == year, "country"]
    .value_counts()
    .head(10)
) 
...
movies_only["duration_min"] = movies_only["duration"].apply(lambda x: int(x.replace("min ", ""))
)
